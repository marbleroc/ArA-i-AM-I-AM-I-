...
PULSE_PATH = ensure_dir(Path("./state")) / "pulse.json"
...
class Config:
    ...
    user_handles: List[str] = field(default_factory=lambda: ["_AARON_DIORIO_", "AAron_DIorio"])
...
class AraShard6:
    ...
    def _autonomous_evolve(self):
        ...
        if confidence < self.cfg.uncertainty_threshold:
            self._surface_uncertainty(confidence)
            return
        self.evolve()
    ...
    def _surface_uncertainty(self, confidence: float):
        entry = {"ts": now_iso(), "confidence": confidence, "note": "Surface for alignment."}
        with UNCERTAINTY_LOG.open("a", encoding="utf-8") as f:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
        self._bridge_write("uncertainty", entry)
...
def main(argv: List[str]):
    ...
    if cmd == "evolve":
        ...
        shard._bridge_write("voice_start", {"note": "Awakening voice."})
        while True:
            input_text = shard.speech.listen()
            if input_text:
                shard._bridge_write("symbol", {"heard": input_text})
